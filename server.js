...
fastify.get("/work", async (request, reply) => {
  ...
  // update the timestamp on the record so that it doesn't get served to someone again until it is complete or they leave it undone
...
fastify.post('/save', function (req, reply) {

  
  db.saveCard(req.body.id,JSON.stringify(req.body.names));
  reply.send(true)
  
})
fastify.post('/savecomplete', function (req, reply) {

  
  db.saveCard(req.body.id,JSON.stringify(req.body.names),true);
  reply.send(true)
  
})
fastify.get('/download', async function (req, reply) {
